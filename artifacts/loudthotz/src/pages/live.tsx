import { useGetLivestreamStatus, useListLivestreamSessions } from "@workspace/api-client-react";
import { Mic2, Users, Calendar, PlayCircle } from "lucide-react";
import { Badge } from "@/components/ui/badge";

export default function LiveReadings() {
  const { data: status, isLoading: statusLoading } = useGetLivestreamStatus();
  const { data: sessions, isLoading: sessionsLoading } = useListLivestreamSessions();
            ...
            <div className="flex-1 rounded-xl overflow-hidden bg-white/5 border border-white/10 aspect-video relative flex flex-col items-center justify-center shadow-2xl">
              {status?.isLive && status.embedUrl ? (
                ...
                <>
                  ...
                  <p className="font-serif text-white/40 text-lg">
                    {status?.scheduledAt 
                      ? `Next reading scheduled for ${new Date(status.scheduledAt).toLocaleString()}` 
              ...
              <div className="bg-white/[0.02] border border-white/5 rounded-xl p-6 flex-1 flex flex-col">
                <div className="mb-6 border-b border-white/5 pb-4">
                  <h3 className="font-display text-2xl font-bold mb-2">
                    {status?.isLive ? status.title : "Loudthotz Open Mic"}
                  </h3>
                  <p className="text-sm text-muted-foreground">
                    {status?.isLive ? status.description : "Join us for our regular live poetry sessions. A space for unbridled expression."}
                  </p>
                  {(status?.season || status?.episode) && (
                    <div className="mt-4 flex gap-2">
                      <Badge variant="outline" className="border-white/10 text-white/60">Season {status.season}</Badge>
